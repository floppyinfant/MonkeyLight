...
#define DEBUG          1 // Debug: print to serial console

#define N_LEDS        40 // Max value is 170 (fits one SD card block)
#define HALL           1 // use HALL-Sensor
...
void loop() {
  ...
  // @totman
  if (HALL) {
    // Set up for HALL-Sensor as trigger
    secPerCycle = (float)(millis() - timeStart) / 1000.0;
    linesPerSec = (float)nBlocks / (float)secPerCycle; // nBlocks / secPerCycle
    if (linesPerSec < nBlocks) {
      linesPerSec = nBlocks;
    }
    timeStart = millis();
  } else {
    // Set up timer based on dial input
    linesPerSec = map(analogRead(SPEED), 0, 1023, 10, maxLPS);
  }
  if (DEBUG) {
    Serial.print(F("linesPerSec: "));
    Serial.print(linesPerSec);
    Serial.print(F(" secPerCycle: "));
    Serial.print(secPerCycle);
  ...
  if(++frame >= nFrames) frame = 0;            // picture
